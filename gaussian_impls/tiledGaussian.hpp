...
__global__ void loadTiledGaussianKernel(float* gaussiankernel, float* gaussiankernel_integral, int gaussiansize, double sigma){
...
struct TiledGaussian{
    ...
    void run(){
        ...
        TiledGaussianBlur_Kernel<<<dim3(bl_x*bl_y), dim3(th_x, th_y)>>>(source, dest, plane.width, plane.height, gaussiankernel_d, gaussiankernel_d+2*windowsize+1);
